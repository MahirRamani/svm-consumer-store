...
export interface ITransactionItem {
  ...
  productId: mongoose.Types.ObjectId; // Required - Product is now the base item
...
export interface ITransaction extends Document {
  ...
  type: "Purchase" | "Topup" | "Deduction" | "Partial Reverted" | "Partial Revert" | "Reverted" | "Revert";
  reason?: string;
...
const TransactionSchema = new Schema<ITransaction>(
  {
    ...
    type: {
      type: String,
      enum: {
        values: ["Purchase", "Topup", "Deduction", "Partial Reverted", "Partial Revert", "Reverted", "Revert"],
...
TransactionSchema.pre("save", function (next) {
  if (this.type === "Purchase" && this.items && this.items.length > 0) {
...
TransactionSchema.statics.getStudentTransactionHistory = async function (
  ...
  return await this.find({ studentId, status: "Completed" })
    .sort({ createdAt: -1 })
    .limit(limit)
    .populate("items.productId", "name size imageURL")
