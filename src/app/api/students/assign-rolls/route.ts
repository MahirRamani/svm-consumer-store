...
// ALL batch students land in currentYear after tx → every finalRoll must be unique
const finalRollList = finalRolls
  .map((r) => r.finalRoll)
...
if (dupRolls.length > 0) {
  ...
  throw new ApiError(
    `These rolls would clash in ${currentYear}: ${[...new Set(conflicting)].join(' | ')}. ` +
  ...
  //     if (prev.year !== currentYear) return true; // crossing into new year with same roll — must check
  ...
  //     //   .join(' | ');
  //     // throw new ApiError(
  //     //   `Roll numbers already taken by active students in ${currentYear}: ${details}`,
  //     //   409
  //     // );
  //     const details = rollConflicts
  //   .map((s) => {
...
const changedRolls = finalRolls
  .filter((r) => {
    ...
    if (prev.year !== currentYear) return true; // crossing into new year
...
if (changedRolls.length > 0) {
  ...
  const rollConflicts = candidates.filter((candidate) => {
    ...
    // In batch — only a conflict if they are KEEPING this roll in currentYear
...
if (dryRun) {
  return successResponse(
    { year: currentYear, report: changeReport },
    200,
...
//         year:        currentYear,
//         reason:      `Roll assignment for ${currentYear}`,
//       };
//     });
...
//     // return successResponse(
//     //   { updated: students.length, year: currentYear },
//     //   200,
//     //   `${students.length} students updated for ${currentYear}.`
//     // );
//     return successResponse(
//   { updated: students.length, year: currentYear, report: changeReport },
//   200,
//   `${students.length} students processed for ${currentYear}.`
// );
  ...
  const auditLogs = studentsToUpdate.map(({ studentMongoId, rollNumber, standard }) => {
    ...
    return {
      ...
      after:  { rollNumber: rollNumber ?? prev.rollNumber, standard: standard ?? prev.standard, year: currentYear, isActive: true },
      performedBy: authContext.user.id,
      year:        currentYear,
      reason:      `Roll assignment for ${currentYear}`,
    };
  ...
  return successResponse(
    { updated: studentsToUpdate.length, skipped: trulyUnchangedIds.size, year: currentYear, report: changeReport },
    200,
...
//           .trim()
//           .optional()
//           .transform((v) => (v?.length ? v : undefined)),  // "" → undefined
...
// // ─── Collision error parser ───────────────────────────────────────────────────
// // Turns:  'E11000 ... dup key: { rollNumber: "830", year: "4" }'
// // Into:   'Roll No "830" already exists in year "4"'

// // const parseCollisionMessage = (errmsg: string): string => {
// //   const dupKeyMatch = errmsg.match(/dup key:\s*\{([^}]+)\}/);
// //   if (!dupKeyMatch) return 'Duplicate key detected';
...
// //   if (parts['rollNumber'] && parts['year']) {
// //     return `Roll No "${parts['rollNumber']}" already exists in year "${parts['year']}"`;
// //   }
// //   if (parts['rollNumber']) {
// //     return `Roll No "${parts['rollNumber']}" already exists`;
// //   }
...
//   const { students } = await validateBody(req, assignRollsSchema);
...
//   // ── 2. Fetch students — no isActive filter ─────────────────────────────────
//   //       Students may be from a previous year; we update them to the new year
//   const existingStudents = await Student.find({ _id: { $in: studentMongoIds } })
//     .select('_id rollNumber standard year name isActive')
//     .lean<StudentLean[]>();
...
//   // ── 3. Roll number conflicts with OTHER students in current year ────────────
//   if (rollNumbers.length > 0) {
//     const rollConflicts = await Student.find({
//       rollNumber: { $in: rollNumbers },
//       year:       currentYear,
//       // isActive:   true,
//       _id:        { $nin: studentMongoIds },  // exclude this batch
//     })
//       .select('rollNumber name')
//       .lean<StudentConflictLean[]>();
...
//         .join(' | ');
//       throw new ApiError(
//         `Roll numbers already taken by other students in ${currentYear}: ${details}`,
//         409
//       );
//     }
//   }
...
//     // // ── DB-level collision — extract meaningful message from errmsg ────────
//     // if (err instanceof MongoBulkWriteError) {
//     //   const writeErrors: WriteError[] = Array.isArray(err.writeErrors)
//     //     ? Array.from(err.writeErrors)
//     //     : [err.writeErrors as WriteError];

//     //   const messages = writeErrors
//     //     .filter((e) => e.code === 11000)
//     //     .map((e) => parseCollisionMessage(e.errmsg ?? ''));

//   // if (err instanceof MongoBulkWriteError) {
//   //   const writeErrors: WriteError[] = Array.isArray(err.writeErrors)
//   //     ? Array.from(err.writeErrors)
//   //     : err.writeErrors != null
//   //     ? [err.writeErrors as WriteError]
//   //     : [];

//   //   const messages = writeErrors
//   //     .filter((e) => {
//   //       // code is a getter — fallback to raw .err.code
//   //       const raw = e as unknown as { err?: { code?: number } };
//   //       return e.code === 11000 || raw.err?.code === 11000;
//   //     })
//   //     .map((e) => {
//   //       // errmsg getter may return undefined — fallback to raw .err.errmsg
//   //       const raw = e as unknown as { err?: { errmsg?: string } };
//   //       const errmsg = e.errmsg ?? raw.err?.errmsg ?? '';
//   //       return parseCollisionMessage(errmsg);
//   //     });
//   //     throw new ApiError(
//   //       messages.length
//   //         ? `Collision detected: ${messages.join('; ')}`
//   //         : 'Duplicate key collision at DB level.',
//   //       409
//   //     );
//   //   }

//     // Terminal shows exactly this structure:
// // WriteError { err: { code: 11000, errmsg: '...', op: {...} } }
...
// //   const messages = writeErrors
// //     .map((e) => {
// //       // Access the raw `err` property directly — this is what terminal shows
// //       const raw = (e as unknown as { err: { code: number; errmsg: string } }).err;
// //       if (raw?.code !== 11000) return null;
// //       return parseCollisionMessage(raw.errmsg ?? '');
// //     })
// //     .filter((m): m is string => m !== null);
...
//     // ❌ instanceof fails across module boundaries — mongoose bundles its own mongodb
//   // if (err instanceof MongoBulkWriteError) { ... }
...
//     const messages = bulkErr.writeErrors
//       .map((e) => {
//         // errmsg lives on e.err — not on e directly (as seen in terminal)
//         if (e.err?.code !== 11000) return null;
//         return parseCollisionMessage(e.err.errmsg ?? '');
//       })
//       .filter((m): m is string => m !== null);
...
// //           .transform((v) => (v?.length ? v : undefined)),   // "" → undefined
// //         standard: z.string().trim().optional(),
// //         id: z.string().trim().regex(/^\d+$/, 'ID must be numeric').optional(),
// //       })
// //     )
// //     .min(1, 'At least one student is required')
// //     .max(500, 'Maximum 500 students per request'),
...
// // // ─── Handler ──────────────────────────────────────────────────────────────────
// // // No idempotency guard — can be called multiple times to fix mistakes.
// // // Uses YearConfig.currentYear — never requires year in body.
// // // Sets isActive: true so students inactive from year-end get re-activated.
...
// //   // ── 0. Get current year + check not locked ─────────────────────────────────
// //   const yearConfig = await YearConfig.findOne()
// //     .select('currentYear isYearLocked')
// //     .lean<YearConfigLean | null>();

// //   if (!yearConfig) {
// //     throw new ApiError(
// //       'No active academic year configured. Please set up the year first.',
// //       400
// //     );
// //   }
// //   if (yearConfig.isYearLocked) {
// //     throw new ApiError(
// //       `Year ${yearConfig.currentYear} is locked (ended). Configure a new year before assigning rolls.`,
// //       409
// //     );
// //   }
...
// //   // ── 2. Duplicate IDs within batch ──────────────────────────────────────────
// //   const incomingIds = students.map((s) => s.id).filter((id): id is string => !!id);
...
// //   // ── 3. Fetch students — NO isActive filter ─────────────────────────────────
// //   //       Students may be inactive from a previous year end; we re-activate them
// //   const existingStudents = await Student.find({ _id: { $in: studentMongoIds } })
// //     .select('_id rollNumber standard year id name isActive')
// //     .lean<StudentLean[]>();
...
// //       year:       currentYear,
// //       isActive:   true,
// //       _id:        { $nin: studentMongoIds },   // exclude this batch (re-assignment allowed)
// //     })
// //       .select('rollNumber name')
// //       .lean<StudentConflictLean[]>();
...
// //         .join(' | ');
// //       throw new ApiError(
// //         `These roll numbers are already taken by other active students in ${currentYear}: ${details}`,
// //         409
// //       );
// //     }
// //   }

// //   // ── 5. ID conflicts with OTHER active students in current year ──────────────
// //   if (incomingIds.length > 0) {
// //     const idConflicts = await Student.find({
// //       id:       { $in: incomingIds },
// //       isActive: true,
// //       year:     currentYear,
// //       _id:      { $nin: studentMongoIds },   // exclude self
// //     })
...
// //         .join(' | ');
// //       throw new ApiError(
// //         `These IDs are already taken by other active students in ${currentYear}: ${details}`,
// //         409
// //       );
// //     }
// //   }
...
// //           update: {
// //             $set: {
// //               rollNumber: rollNumber || prev.rollNumber,   // || catches "" + undefined
// //               standard:   standard   || prev.standard,
// //               year:       currentYear,
// //               isActive:   true,                            // re-activate from year-end
// //               ...(id ? { id } : {}),
// //             },
// //           },
// //         },
...
// //     // Audit log — before/after per student
...
// //         performedBy: authContext.user.id,
// //         year:        currentYear,
// //         reason:      `Roll number assignment for ${currentYear}`,
// //       };
// //     });
