...
//   // Create stocks map
//   const subProductStocks = useMemo(() => {
//     const stocksMap = new Map<string, StockInfo>();
...
//         })),
//       }));
//   }, [productsResponse, subProductStocks]);
...
//   const getFirstActiveVariant = useCallback((product: ProductWithVariants): SubProductWithStock | null => {
//     if (!product.variants || product.variants.length === 0) return null;
//     const variant = product.variants.find(v => v.isActive);
//     if (!variant) return null;
...
//   const getPriceRange = useCallback((product: ProductWithVariants): { min: number; max: number } | null => {
//     if (!product.variants || product.variants.length === 0) return null;
...
//   // Refresh single stock
//   const refreshStockForSubProduct = useCallback(async (subProductId: string) => {
//     setRefreshingStocks(prev => new Set(prev).add(subProductId));
...
//               const stocksArray = [...updatedData.data.data];
...
//                 updatedData.data.data = stocksArray;
//                 toast.success("Stock refreshed - new batch loaded");
//               } else {
//                 if (index >= 0) {
//                   stocksArray.splice(index, 1);
...
//   // Refresh depleted stocks
//   const refreshDepletedStocks = useCallback(async () => {
//     const depletedSubProductIds: string[] = [];
...
//   // Update stock in cache after adding to cart
//   const updateStockInCache = useCallback((subProductId: string, quantityReduced: number) => {
//     queryClient.setQueryData<ApiResponse<StockFIFOResponse>>(
//       ["fifo-stocks"],
//       (oldData) => {
...
//   const handleInitialAddToCart = useCallback((product: ProductWithVariants) => {
//     setActiveProducts(prev => ({ ...prev, [product._id]: true }));
//     setQuantities(prev => ({ ...prev, [product._id]: 1 }));
//   }, []);

//   const handleQuantityChange = useCallback((productId: string, newQuantity: number, maxStock: number) => {
//     if (newQuantity < 1) {
//       setQuantities(prev => ({ ...prev, [productId]: 1 }));
//       return;
//     }
...
//   const handleQuantityClick = useCallback((productId: string, currentQuantity: number) => {
//     setEditingProductId(productId);
//     setEditingValue(currentQuantity.toString());
//   }, []);
...
//   const handleQuantityBlur = useCallback((productId: string, maxStock: number) => {
...
//   const handleFinalAddToCart = useCallback((product: ProductWithVariants) => {
//     const firstActiveVariant = getFirstActiveVariant(product);
//     if (firstActiveVariant && firstActiveVariant.currentStock) {
//       const quantity = quantities[product._id] || 1;
...
//   const handleAddToCart = useCallback((product: ProductWithVariants) => {
//     const variantCount = product.variants?.length || 0;
...
//   const handleVariantAddToCart = useCallback((variant: SubProductWithStock, quantity: number) => {
//     const stockInfo = subProductStocks.get(variant._id);
//     if (!stockInfo) {
//       toast.error("No stock information available");
//       return;
...
//   const getProductsCountByCategory = useCallback((categoryId: string): number => {
//     if (categoryId === "all") {
//       return products.filter((product) =>
//         product.name.toLowerCase().includes(searchTerm.toLowerCase())
//       ).length;
...
//             const firstActiveVariant = getFirstActiveVariant(product);
//             const priceRange = getPriceRange(product);
//             const isActive = activeProducts[product._id];
//             const quantity = quantities[product._id] || 1;
//             const isEditing = editingProductId === product._id;
//             const isRefreshing = firstActiveVariant && refreshingStocks.has(firstActiveVariant._id);
//             const variantCount = product.variants?.length || 0;
...
//                         <span
//                           className={`text-sm font-medium ${
//                             firstActiveVariant.currentStock && hasLowStock(firstActiveVariant)
//                               ? "text-red-600"
//                               : "text-gray-700"
//                           }`}
//                         >
...
//                         <Package className="w-4 h-4 text-gray-500" />
//                         <span className="text-sm font-medium text-gray-700">
//                           {product.variants?.filter(v => v.isActive && subProductStocks.get(v._id)).length || 0} in stock
//                         </span>
//                       </div>
//                       <Badge variant="outline" className="text-xs">
//                         {product.categoryName}
...
//                             variant="outline"
//                             size="sm"
//                             onClick={() => handleQuantityChange(product._id, quantity - 1, firstActiveVariant.currentStock!.quantityLeft)}
//                             disabled={quantity <= 1}
//                             className="h-7 w-7 p-0"
//                           >
//                             <Minus className="h-3 w-3" />
...
//                               value={editingValue}
//                               onChange={handleManualQuantityChange}
//                               onBlur={() => handleQuantityBlur(product._id, firstActiveVariant.currentStock!.quantityLeft)}
//                               onKeyDown={(e) => handleQuantityKeyPress(e, product._id, firstActiveVariant.currentStock!.quantityLeft)}
//                               className="w-14 h-7 text-center text-sm p-1"
//                               autoFocus
//                             />
//                           ) : (
...
// // import { toast } from "sonner";
// // import ProductVariantsModal from "@/components/modals/product-variants-modal";
// // import type { Product, CartItem, SubProduct, Category } from "@/types";
...
// // interface SubProductWithStock extends SubProduct {
...
// //   // Fetch FIFO stocks
// //   const { data: stocksData } = useQuery<Map<string, StockInfo>>({
// //     queryKey: ["fifo-stocks"],
// //     queryFn: async (): Promise<Map<string, StockInfo>> => {
// //       const response = await fetch("/api/stock/fifo/oldest-all");
// //       if (!response.ok) throw new Error("Failed to fetch stocks");
...
// //       // Fix: Check if data exists and is an array before iterating
// //       if (result.data && Array.isArray(result.data)) {
// //         result.data.forEach((item: StockItem) => {
// //           stocksMap.set(item.subProductId, {
...
// //   const subProductStocks = stocksData || new Map<string, StockInfo>();

// //   // Refresh stock for single sub-product
// //   const refreshStockForSubProduct = useCallback(
// //     async (subProductId: string) => {
// //       setRefreshingStocks((prev) => new Set(prev).add(subProductId));
...
// //           queryClient.setQueryData(["fifo-stocks"], (oldData: Map<string, StockInfo> | undefined) => {
// //             const updated = new Map(oldData || new Map());

// //             if (result.data) {
// //               updated.set(subProductId, {
// //                 stockTransactionId: result.data.stockTransactionId,
// //                 sellingPrice: result.data.sellingPrice,
// //                 quantityLeft: result.data.quantityLeft,
// //                 stockDate: result.data.stockDate,
// //               });
// //               toast.success("Stock refreshed - new batch loaded");
// //             } else {
// //               updated.delete(subProductId);
...
// //   // Update stock in cache after adding to cart
// //   const updateStockInCache = useCallback(
// //     (subProductId: string, quantityReduced: number) => {
// //       queryClient.setQueryData(["fifo-stocks"], (oldData: Map<string, StockInfo> | undefined) => {
// //         const updated = new Map(oldData || new Map());
// //         const currentStock = updated.get(subProductId);
...
// //   const categories = useMemo(
// //     () => (Array.isArray(categoriesData) ? categoriesData.filter((category) => category.isActive === true) : []),
// //     [categoriesData]
// //   );
...
// //     return productsData
// //       .filter((product) => product.isActive === true) // Fix: explicitly check === true
// //       // .filter((product) => product.isActive === true || product.isActive === null)
// //       .map((product) => ({
// //         ...product,
// //         variantCount: product.variants?.length || 0,
// //         categoryId: typeof product.category === "object" && product.category !== null ? product.category.id : product.categoryId,
// //         categoryName: typeof product.category === "object" && product.category !== null ? product.category.name : getCategoryName(product.categoryId),
// //         variants: product.variants?.map((variant) => ({
// //           ...variant,
// //           currentStock: subProductStocks.get(variant.id) || null,
// //         })),
// //       }));
// //   }, [productsData, subProductStocks]);
...
// //   const getFirstActiveVariant = useCallback(
// //     (product: ProductWithStock): SubProductWithStock | null => {
// //       if (!product.variants || product.variants.length === 0) return null;
// //       const variant = product.variants.find((v) => v.isActive !== false); // Fix: explicitly check === true
// //       if (!variant) return null;
...
// //   const getPriceRange = useCallback(
// //     (product: ProductWithStock): { min: number; max: number } | null => {
// //       if (!product.variants || product.variants.length === 0) return null;
// //       const activePrices = product.variants
// //         .filter((variant) => variant.isActive === true && subProductStocks.get(variant.id)) // Fix: explicitly check === true
// //         .map((variant) => subProductStocks.get(variant.id)?.sellingPrice || 0)
// //         .filter((price) => price > 0);
...
// //       if (!variant.currentStock) return true;
// //       return variant.currentStock.quantityLeft <= (variant.lowStockThreshold || 5);
// //     },
// //     []
// //   );
...
// //   const handleInitialAddToCart = useCallback((product: ProductWithStock) => {
// //     setActiveProducts((prev) => ({ ...prev, [product.id]: true }));
...
// //   const handleQuantityKeyPress = useCallback(
// //     (e: React.KeyboardEvent, productId: string, maxStock: number) => {
// //       if (e.key === "Enter") {
// //         handleQuantityBlur(productId, maxStock);
// //       } else if (e.key === "Escape") {
// //         setEditingProductId(null);
...
// //         updateStockInCache(firstActiveVariant.id, quantity);
...
// //         setActiveProducts((prev) => ({ ...prev, [product.id]: false }));
// //         setQuantities((prev) => ({ ...prev, [product.id]: 1 }));
// //         return;
// //       }
...
// //   const handleVariantAddToCart = useCallback(
// //     (variant: SubProduct, quantity: number) => {
...
// //       updateStockInCache(variant.id, quantity);
...
// //     (categoryId: string): number => {
// //       if (categoryId === "all") {
// //         return products.filter((product) => product.name.toLowerCase().includes(searchTerm.toLowerCase())).length;
// //       }
// //       return products.filter(
// //         (product) => product.categoryId === categoryId && product.name.toLowerCase().includes(searchTerm.toLowerCase())
// //       ).length;
// //     },
// //     [products, searchTerm]
// //   );
...
// //         <h2 className="text-2xl font-bold text-gray-900">Products</h2>
// //         <div className="flex items-center gap-2">
// //           <Button onClick={refreshDepletedStocks} variant="outline" size="sm" className="flex items-center gap-2">
// //             <RefreshCw className="w-4 h-4" />
// //             Refresh Depleted
// //           </Button>
// //           <Badge variant="outline" className="text-sm">
...
// //               <div className="relative">
// //                 <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
// //                 <Input placeholder="Product name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
// //               </div>
// //             </div>
// //             <div>
...
// //           <TabsTrigger
// //             value="all"
// //             className="flex items-center space-x-2 py-2 px-4 bg-white data-[state=active]:bg-blue-100 data-[state=active]:shadow-sm"
// //           >
// //             <span>All Categories</span>
// //             <Badge variant="secondary" className="ml-1 text-xs">
// //               {getProductsCountByCategory("all")}
...
// //             const firstActiveVariant = getFirstActiveVariant(product);
// //             const priceRange = getPriceRange(product);
// //             const isActive = activeProducts[product.id];
// //             const quantity = quantities[product.id] || 1;
// //             const isEditing = editingProductId === product.id;
// //             const isRefreshing = firstActiveVariant && refreshingStocks.has(firstActiveVariant.id);
// //             // const isRefreshing = !!(firstActiveVariant && refreshingStocks.has(firstActiveVariant.id));
// //             const variantCount = product.variants?.length || 0;
...
// //                     <div className="flex flex-col items-end gap-1">
// //                       {variantCount === 1 && firstActiveVariant?.currentStock ? (
// //                         <span className="text-lg font-bold text-green-600">₹{firstActiveVariant.currentStock.sellingPrice?.toFixed(2)}</span>
// //                       ) : variantCount > 1 && priceRange ? (
// //                         <div className="text-right">
// //                           <span className="text-lg font-bold text-green-600">
// //                             ₹{priceRange.min.toFixed(2)}
// //                             {priceRange.min !== priceRange.max && ` - ₹${priceRange.max.toFixed(2)}`}
// //                           </span>
// //                           <Badge variant="secondary" className="mt-1 block text-xs">
// //                             {variantCount} Variants
// //                           </Badge>
// //                         </div>
// //                       ) : (
...
// //                       {variantCount === 1 && firstActiveVariant && (
// //                         <Button
// //                           onClick={() => refreshStockForSubProduct(firstActiveVariant.id)}
// //                           disabled={isRefreshing}
// //                           variant="ghost"
// //                           size="sm"
// //                           className="h-6 px-2"
...
// //                         <span
// //                           className={`text-sm font-medium ${
// //                             firstActiveVariant.currentStock && hasLowStock(firstActiveVariant) ? "text-red-600" : "text-gray-700"
// //                           }`}
// //                         >
// //                           Stock: {firstActiveVariant.currentStock?.quantityLeft || 0}
// //                         </span>
...
// //                         <Package className="w-4 h-4 text-gray-500" />
// //                         <span className="text-sm font-medium text-gray-700">
// //                           {product.variants?.filter((v) => v.isActive !== false && subProductStocks.get(v.id)).length || 0} in stock
// //                         </span>
// //                       </div>
// //                       <Badge variant="outline" className="text-xs">
// //                         {product.categoryName}
...
// //                   <div className="space-y-2">
// //                     {variantCount > 1 ? (
// //                       <Button onClick={() => handleAddToCart(product)} className="w-full bg-blue-500 hover:bg-blue-600 text-white">
// //                         <Eye className="w-4 h-4 mr-2" />
// //                         View Variants
// //                       </Button>
// //                     ) : firstActiveVariant && !isActive ? (
...
// //                             variant="outline"
// //                             size="sm"
// //                             onClick={() => handleQuantityChange(product.id, quantity - 1, firstActiveVariant.currentStock!.quantityLeft)}
// //                             disabled={quantity <= 1}
...
// //                               value={editingValue}
// //                               onChange={handleManualQuantityChange}
// //                               onBlur={() => handleQuantityBlur(product.id, firstActiveVariant.currentStock!.quantityLeft)}
// //                               onKeyDown={(e) => handleQuantityKeyPress(e, product.id, firstActiveVariant.currentStock!.quantityLeft)}
// //                               className="w-16 h-8 text-center text-sm p-1"
// //                               autoFocus
// //                             />
// //                           ) : (
// //                             <span
// //                               className="w-16 text-center font-medium cursor-pointer hover:bg-gray-100 rounded px-2 py-1 text-sm"
// //                               onClick={() => handleQuantityClick(product.id, quantity)}
// //                               title="Click to edit quantity"
// //                             >
// //                               {quantity}
// //                             </span>
...
// //                           </Button>
// //                         </div>
// //                         <Button onClick={() => handleFinalAddToCart(product)} className="w-full bg-green-500 hover:bg-green-600 text-white text-sm">
// //                           <ShoppingCart className="w-4 h-4 mr-2" />
