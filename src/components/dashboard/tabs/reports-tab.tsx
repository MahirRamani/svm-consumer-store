...
type ActiveTab = "sold" | "stock" | "student";
...
// Student expense report types
// Backend API: GET /api/reports/student-expense?fromDate=YYYY-MM-DD&toDate=YYYY-MM-DD
// Returns StudentExpenseResponse (shape defined below)
interface StudentExpenseRow {
  studentId: string;         // IStudent._id
  rollNumber: string;        // IStudent.rollNumber
  name: string;              // IStudent.name
  standard: string;          // IStudent.standard
  currentBalance: number;    // IStudent.balance (live)
...
interface StudentExpenseResponse {
  data: StudentExpenseRow[];
  summary?: {
...
const getQuickRangeDates = (range: QuickRange): { from: string; to: string } => {
  const t = new Date();
  ...
  switch (range) {
    ...
    case "this_week": {
      ...
      return { from: fmt(mon), to: fmt(t) };
    }
    case "this_month": {
      const first = new Date(t.getFullYear(), t.getMonth(), 1);
      return { from: fmt(first), to: fmt(t) };
    }
    case "last_month": {
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-1.5 ${alignClass}`}>
        <span>{label}</span>
        <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? "text-indigo-400" : "text-gray-300"}`} />
      </div>
    </th>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Compact inline stat chip — used inside the filter card
// ─────────────────────────────────────────────────────────────────────────────
...
const StatChip = ({ icon, label, value, accent = "#6366f1" }: StatChipProps) => (
  <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-slate-100 bg-white shadow-sm min-w-0">
    <div className="rounded-lg p-1.5 flex-shrink-0" style={{ background: `${accent}18` }}>
      <span style={{ color: accent }} className="block [&>svg]:w-4 [&>svg]:h-4">{icon}</span>
    </div>
    <div className="min-w-0">
      <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider leading-none">{label}</p>
      <p className="text-base font-bold text-slate-800 leading-tight mt-0.5 truncate">{value}</p>
    </div>
  </div>
);
...
// ─────────────────────────────────────────────────────────────────────────────
// SoldBatchRows — numbered like stock (no batch hash)
// ─────────────────────────────────────────────────────────────────────────────
...
const StockBatchRows = ({ batches }: { batches: StockBatch[] }) => (
  <>
    {batches.map((b, i) => {
      ...
      return (
        <tr key={b.batchId} className="bg-slate-50" style={{ borderLeft: `4px solid ${borderColor}` }}>
          <td className="pl-14 pr-4 py-2.5 text-xs text-slate-500 font-mono" colSpan={2}>
            Batch {i + 1}
            <span className="ml-3 text-slate-400">{formatDate(b.purchaseDate)}</span>
          </td>
          ...
          <td className="px-6 py-2.5 text-center text-xs font-mono text-slate-500">{b.soldQuantity}</td>
          <td className="px-6 py-2.5 text-center text-xs text-slate-400 font-mono">
...
// ─────────────────────────────────────────────────────────────────────────────
// DateRangeControls — shared across tabs
// ─────────────────────────────────────────────────────────────────────────────
...
const DateRangeControls = ({
  quickRange, fromDate, toDate, onQuickRange, onFromDate, onToDate,
      ...
      {quickRange === "custom" && (
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-40 text-sm px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400"
          />
...
export default function InventoryReportDashboard() {
  ...
  const { data: soldResponse, isLoading: soldLoading, isFetching: soldFetching } =
    useQuery<SoldReportResponse>({
      queryKey: ["report-sold", fromDate, toDate],
      queryFn: async (): Promise<SoldReportResponse> => {
        const res = await fetch(`/api/reports/sold?fromDate=${fromDate}&toDate=${toDate}`);
  ...
  const { data: stockResponse, isLoading: stockLoading, isFetching: stockFetching } =
    useQuery<StockReportResponse>({
      queryKey: ["report-stock", fromDate, toDate],
      queryFn: async (): Promise<StockReportResponse> => {
        const res = await fetch(`/api/reports/stock?fromDate=${fromDate}&toDate=${toDate}`);
  ...
  // ── Fetch student expense report ──────────────────────────────────────────
  const { data: studentResponse, isLoading: studentLoading, isFetching: studentFetching } =
    useQuery<StudentExpenseResponse>({
      queryKey: ["report-student", fromDate, toDate],
      queryFn: async (): Promise<StudentExpenseResponse> => {
        const res = await fetch(`/api/reports/student-expense?fromDate=${fromDate}&toDate=${toDate}`);
        if (!res.ok) throw new Error("Failed to fetch student expense report");
        return res.json();
      },
  ...
  const handleSoldSort = useCallback((key: SoldSortKey) => {
    setSoldSort((prev) => ({ key, dir: prev.key === key && prev.dir === "asc" ? "desc" : "asc" }));
    setExpandedRows({});
  }, []);
  ...
  const handleStudentSort = useCallback((key: StudentSortKey) => {
      ...
      switch (stockSort.key) {
        ...
        case "newestBatch": {
          const aDate = a.batches.length > 0
            ? Math.max(...a.batches.map((bx) => new Date(bx.purchaseDate).getTime()))
            : 0;
          const bDate = b.batches.length > 0
            ? Math.max(...b.batches.map((bx) => new Date(bx.purchaseDate).getTime()))
            : 0;
          cmp = aDate - bDate;
          break;
        }
  ...
  const sortedStudents = useMemo<StudentExpenseRow[]>(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (studentSort.key) {
        ...
        case "topupInPeriod":
          ...
          break;
        case "expenseInPeriod":
          cmp = a.expenseInPeriod - b.expenseInPeriod;
          break;
        case "currentBalance":
  ...
  }, [studentResponse, searchQuery, studentSort]);
    ...
    try {
      ...
      if (activeTab === "sold") {
        const headers = [
          ...
          "Batch No", "Batch Purchase Date", "Buying Price (₹)", "Qty from Batch", "Revenue from Batch (₹)",
        ];
        const rows: string[] = [];
        ...
        const headers = [
          ...
          "Batch No", "Batch Purchase Date", "Buying Price (₹)", "Initial Qty", "Remaining Qty", "Sold Qty", "Batch Status",
        ];
        const rows: string[] = [];
        ...
        const headers = [
          ...
          "Last Period Balance (₹)", "Topup in Period (₹)", "Expense in Period (₹)", "Current Balance (₹)",
          "Refund",
        ];
        ...
        // Add a header row with period info
        csvContent = [
          `"Student Expense Report — ${rangeLabel}"`,
          headers.join(","),
          ...rows,
          "",
  ...
  }, [activeTab, sortedSold, sortedStock, sortedStudents, fromDate, toDate, quickRange, studentSummary]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Sold products, remaining stock &amp; student expenses with batch-level detail
          </p>
        </div>
        <div className="flex items-center gap-3">
          {isFetching && !isLoading && <Loader2 className="w-4 h-4 animate-spin text-blue-500" />}
          <Button onClick={handleExport} variant="outline" disabled={exportDisabled}>
            {isExporting
      ...
      {/* ── Filter Card — date range + search LEFT | stats RIGHT ── */}
      <Card>
          ...
          <div className="flex flex-col xl:flex-row gap-5 xl:items-end">
            ...
            <div className="flex flex-col gap-4 flex-1 min-w-0">
              ...
              <div className="flex items-end gap-4">
                ...
                <p className="text-xs text-gray-400 pb-2.5 whitespace-nowrap">
                  ...
                  {soldLoading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-gray-400">
                        No products sold in this date range with current filters.
                      </td>
                    </tr>
                  ) : (
                    sortedSold.map((row) => {
        ...
        <Card>
          <CardHeader className="pb-0">
            <CardTitle className="text-base font-semibold text-gray-900 flex items-center gap-2">
              ...
              {studentFetching && !studentLoading && (
                <Loader2 className="w-4 h-4 animate-spin text-blue-400 ml-auto" />
              )}
            </CardTitle>
          </CardHeader>
                  ...
                  {studentLoading ? (
                    ...
                    <tr>
                      <td colSpan={8} className="px-6 py-12 text-center text-gray-400">
                        ...
                        No student data found for this period.
                      </td>
                    </tr>
                  ) : (
                      ...
                      <tr key={row.studentId ?? `student-row-${row.rollNumber}`} className="hover:bg-gray-50 transition-colors">
                        {/* Roll No */}
                        <td className="px-6 py-3.5 font-mono text-sm text-gray-700 font-semibold">
                          {row.rollNumber}
                        </td>
                        {/* Name */}
                        <td className="px-6 py-3.5 font-medium text-gray-900">
                        ...
                        {/* Refund — intentionally empty */}
                        <td className="px-6 py-3.5 text-center text-gray-300">—</td>
                      </tr>
                    ))
                  )}
                </tbody>
                {!studentLoading && sortedStudents.length > 0 && (
                  <tfoot>
                    <tr className="bg-gray-50 border-t-2 border-gray-200 font-semibold text-sm">
                      <td className="px-6 py-3.5 text-gray-700" colSpan={3}>
                      ...
                      <td className="px-6 py-3.5 text-right text-red-600 font-mono">
                        -{formatINR(studentSummary.totalExpense)}
                      </td>
                      <td className="px-6 py-3.5 text-right text-gray-800 font-bold font-mono">
                        {formatINR(studentSummary.totalBalance)}
                      </td>
                      <td />
                    </tr>
                  </tfoot>
