...
export default function ShoppingCart({
  ...
  >({
    ...
    onSuccess: (response) => {
      ...
      // Access the transaction ID from the correct path
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Success Modal - Handle close on outside click */}
      <SuccessModal
        open={showSuccessModal}
        onOpenChange={(open) => {
          setShowSuccessModal(open);
          // If modal is being closed (by any means), trigger the transaction complete handler
          if (!open) {
            onTransactionComplete();
          }
        }}
